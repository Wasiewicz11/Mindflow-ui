...
export interface SidebarContentProps {
  activeTab: 'dashboard' | 'inbox' | 'notes' | 'tasks' | 'goals' | 'insights' | 'brain' | 'calendar' | 'settings';
  setActiveTab: (tab: 'dashboard' | 'inbox' | 'notes' | 'tasks' | 'goals' | 'insights' | 'brain' | 'calendar' | 'settings') => void;
  user: User | null;
  spaces: Space[];
  projects: Project[];
...
export const SidebarContent: React.FC<SidebarContentProps> = ({
  ...
  const menuItems: Array<{ id: ActiveTab; label: string; icon: React.ReactNode }> = [
    ...
    {
      id: 'inbox', label: 'Powiadomienia', icon: <Bell className="h-5 w-5" />
    },
    {
